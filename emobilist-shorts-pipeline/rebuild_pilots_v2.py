...
def delete_video(channel: str, video_id: str, *, dry_run: bool = False):
    """HARD SAFETY: only delete agent-owned IDs with viewCount <= 3.

    See youtube_delete_safety.py / YOUTUBE_DELETE_SAFETY.md.
    """
    try:
        from youtube_delete_safety import safe_delete_video
    except ImportError:
        import sys
        from pathlib import Path
    ...
    result = safe_delete_video(channel, video_id, dry_run=dry_run)

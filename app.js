...
const GAMES = [
  ...
  { id: 'race', title: 'Word Kart', ico: '🏎️', blurb: 'Tilt to steer. Drive over sight words and hear them out loud!', skill: 'Sight words', unlock: 0 },
...
const GAME_HOW = {
  ...
  race: 'Word Kart. Your kart zooms by itself. Tilt the iPad left and right to steer. Drive over glowing sight words. Each word you hit is read out loud.',
...
function showView(id) {
  ...
  if (id !== 'race' && typeof stopRace === 'function') stopRace()
  ...
  if (id === 'race') setTimeout(() => { if (typeof sizeRaceCanvas === 'function') sizeRaceCanvas() }, 40)
}
...
function openGame(id) {
  ...
  if (id === 'race') {
    showView('race')
    speakHowTo('race').then(() => {
      if (typeof startRaceGame === 'function') startRaceGame()
    })
    return
  }
...
async function init() {
  ...
  document.getElementById('raceBack')?.addEventListener('click', () => { if (typeof stopRace === 'function') stopRace(); showView('games') })
  document.getElementById('raceNext')?.addEventListener('click', () => { if (typeof startRaceGame === 'function') startRaceGame() })
  document.getElementById('raceGyroBtn')?.addEventListener('click', () => { if (typeof requestRaceGyro === 'function') requestRaceGyro() })
  bindSlicePointer()
